...
class SQLConversation implements ConversationInterface
{
    ...
    public function get_chats(): array {
        $table_name = $this->db->prefix . 'it_ai_assistant101_conversations';
        $stmt = $this->db->query( "SELECT id, title, mode, model FROM $table_name ORDER BY id DESC" );
    ...
    public function find( int $chat_id ): self|null {
        $table_name = $this->db->prefix . 'it_ai_assistant101_conversations';
        $stmt = $this->db->prepare( "SELECT * FROM $table_name WHERE id = :chat_id" );
        $stmt->execute( [
            ":chat_id" => $chat_id,
        ] );
    ...
    public function get_messages(): array {
        ...
        $table_name = $this->db->prefix . 'it_ai_assistant101_messages';

        $stmt = $this->db->prepare( "SELECT * FROM $table_name WHERE `conversation` = :chat_id" );
        $stmt->execute( [
    ...
    public function save(): int {
        if( ! isset( $this->chat_id ) ) {
            ...
            $stmt = $this->db->prepare( "UPDATE $table_name SET title = :title, mode = :mode, model = :model WHERE id = :chat_id LIMIT 1" );
            $stmt->execute( [
                ":title" => $this->title,
                ":mode" => $this->mode,
                ":model" => $this->model,
    ...
    public function delete(): void {
        ...
        $stmt = $this->db->prepare( "DELETE FROM $table_name WHERE conversation = :chat_id" );
        $stmt->execute([
            ":chat_id" => $this->chat_id,
        ]);
        ...
        $stmt = $this->db->prepare( "DELETE FROM $table_name WHERE id = :chat_id LIMIT 1" );
        $stmt->execute([
            ":chat_id" => $this->chat_id,
        ]);
    }
